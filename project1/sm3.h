...
string padding(string str);//对数据填充的函数实现
...
string MAdd(string str1, string str2);//mod 2^32运算的函数实现
...
string MAdd(string str1, string str2) {
	string res1 = HexToBin(str1);
	string res2 = HexToBin(str2);
	char temp = '0';
	string res = "";
...
string compress(string str1, string str2) {
	...
	for (int j = 0; j < 64; j++) {
		SS1 = LeftShift(MAdd(MAdd(LeftShift(A, 12), E), LeftShift(T(j), (j % 32))), 7);
		SS2 = XOR(SS1, LeftShift(A, 12));
		TT1 = MAdd(MAdd(MAdd(FF(A, B, C, j), D), SS2), str1.substr((j + 68) * 8, 8));
		TT2 = MAdd(MAdd(MAdd(GG(E, F, G, j), H), SS1), str1.substr(j * 8, 8));
		D = C;
		C = LeftShift(B, 9);
		B = A;
		A = TT1;
